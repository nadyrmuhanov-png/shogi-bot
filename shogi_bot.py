...
def draw_piece_card(piece):
    """Генерирует красивую карточку фигуры"""
    W, H = 400, 420
    ...
    try:
        ...
        font_ru    = ImageFont.truetype("arial.ttf", 22)
    ...
    # Фон фигуры — деревянный квадрат
    margin = 40
    draw.rounded_rectangle([margin, margin, W - margin, 280],
                           radius=20, fill=(210, 160, 80), outline=(139, 90, 43), width=3)

    # Иероглиф / эмодзи фигуры
    cx = W // 2
    draw.text((cx, 160), piece["emoji"], font=font_big, fill=(30, 30, 30), anchor="mm")

    # Японское название
    draw.text((cx, 310), piece["name"].split(" ")[0], font=font_name,
              fill=(80, 40, 0), anchor="mm")

    # Русское название
    draw.text((cx, 355), piece["ru_name"], font=font_ru, fill=(100, 60, 0), anchor="mm")

    # Подпись внизу
    draw.text((cx, 395), "将棋 • Сёги", font=font_small, fill=(160, 120, 60), anchor="mm")
